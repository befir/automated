...
public class EntryTermsManage extends PageAutomatedTest {

    @Test
    public void gotoEntryTermsManagePage() throws InterruptedException {
//        driverQuite = false;
        ...
        List<WebElement> elements=driver.findElement(By.linkText("商家中心")).findElements(By.xpath("//li[contains(@definedattr,'/back-merchant-web/index.html#/entryTermsList')]"));
    ...
    //新增入驻条款
    @Test
    public void addEntryTerms() throws InterruptedException {
        gotoEntryTermsManagePage();
        Actions action =new Actions(driver);
        driver.switchTo().frame(driver.findElement(By.id("300819frame")));
        driverWait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//button")));
        Thread.sleep(1000);
        action.click(driver.findElement(By.xpath("//button[contains(@ng-click,'addNew')]"))).perform();
        JSONObject entryTerms = new JSONObject();
        entryTerms.put("title","入驻条款"+dateFormat.format(new Date()));
        entryTerms.put("entryType",1);//1：商家条款  2：店铺条款
        entryTerms.put("status",0);//0：禁用   1：启用
        entryTerms.put("note","条款备注"+dateFormat.format(new Date()));
        String content="第一条\n" +
                "\n" +
                ...
                "第五条 当当网\n" +
                "\n" +
                "在本规定中指\"北京当当网信息技术有限公司\"与\"北京当当科文电子商务有限公司\"及其他关联公司共同运的网站，域名为www.dangdang.com。根据上下文，\"当当\"也可泛指该网站经营主体。\n" +
                "\n" +
                "第六条 商户\n" +
                "\n" +
                "指在当当开设网络店铺独立经营、从事网上商品和服务销售的合作者。\n" +
                "\n" +
                "第七条 店铺\n" +
                "\n" +
                ...
                "第十六 公告\n" +
                "\n" +
                "入驻商家公告规范：需在店铺首页最顶部以及单品也公共栏目公告店铺相关信息。\n" +
                "\n" +
                "\n" +
                "\n" +
                "\n" +
                ...
                "\n" +
                "第二十四条 当当网站对商家的一般违规行为采取以下违规处理方式：\n" +
                "\n" +
                ...
                "（二）违约\n" +
                "\n" +
                "若顾客投诉商家商品描述不符，当当客服按以上判定属于描述不符的，将商品下架商品并按照订单商品实际成交金额的30%向顾客支付违约金，金额最低20元最高不超过500元。违约金由当当以先行赔付形式给顾客，后续从商家货款中扣回；\n" +
                "\n" +
                "\n" +
                "\n" +
                "违背第11项：\n" +
                ...
                "\n" +
                "2、发布非当当购物链接、实体店信息、银行账号及个人支付宝账户等信息，特殊情形除外；\n" +
                "\n" +
                ...
                "\n" +
                "2、商品正标题、副标题、商品介绍中如出现绝对化用语、排序词包括但不限于以下，且无法证明的按照《虚假宣传》处以退一赔三，最低500元赔付顾客：\n" +
                "\n" +
                ...
                "\n" +
                "4、出现第（四）项第1点极限用词，如被举报，由商户自行承担法律责任以及行政处罚。参照（2015年9月1日执行的新《中华人民共和国广告法》）\n" +
                "\n" +
                "第二十九条 经营范围规定\n" +
                "\n" +
                "（一）定义和认定\n" +
                "\n" +
                "百货服装品类商户，不得跨品售卖出版物商品；出版物商户不得售卖百货服装类商品。\n" +
                "\n" +
                "（二）违约：\n" +
                "\n" +
                "商家违背的每次扣12分。\n" +
                "\n";
        driver.findElement(By.id("ueditor_0")).sendKeys(content);//给id为ueditor_0的元素赋值
        Thread.sleep(1000);
        AngularScope.appendData(driver,"document.getElementsByClassName('panel panel-body')","entryTerms",entryTerms);
        Thread.sleep(1000);
        action.click(driver.findElement(By.xpath("//button[contains(@ng-click,'saveEntryTerms()')]"))).perform();
        Thread.sleep(1000);
    }
    //编辑入驻条款
    @Test
    public void editEntryTerms() throws InterruptedException {
        gotoEntryTermsManagePage();
        Actions action =new Actions(driver);
        driver.switchTo().frame(driver.findElement(By.id("300819frame")));
        driverWait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//button")));
        Thread.sleep(1000);
        action.click(driver.findElement(By.xpath("//button[contains(@ng-click,'editEntryTerms(entryTerms)')]"))).perform();
        JSONObject entryTerms = new JSONObject();
        entryTerms.put("title","条款"+dateFormat.format(new Date()));
        entryTerms.put("entryType",1);//1：商家条款  2：店铺条款
        entryTerms.put("status",0);//0：禁用   1：启用
        entryTerms.put("note","条款备注"+dateFormat.format(new Date()));
        String content="第一条\n" +
                ...
                "商家违背的每次扣12分。\n" +
                "\n";
//        driver.findElement(By.id("ueditor_0")).clear();
//        driver.findElement(By.id("ueditor_0")).sendKeys(content);
        Thread.sleep(1000);
        AngularScope.appendData(driver,"document.getElementsByClassName('panel panel-body')","entryTerms",entryTerms);
        Thread.sleep(1000);
        action.click(driver.findElement(By.xpath("//button[contains(@ng-click,'saveEntryTerms()')]"))).perform();
